...
4. **The durable Core-tool conversion is underway but not complete.** Emergency Fund, Debt Payoff Planner, Insurance Gap Checker, Paycheck Planner, Savings Goals, and Open Enrollment Comparison now save and restore completed outcomes. Paycheck Planner preserves the member's chosen allocation, Savings Goals can create or replace only its own valid Life Goals after explicit confirmation, and Open Enrollment preserves a selected plan without implying enrollment. Other Core calculators still need this pattern.
...
| Core money tools | 86 | B design | Broad calculation coverage; six saved/restorable Core outcomes; explicit Paycheck and Open Enrollment choices; confirmed Savings Goals-to-Life Goals handoff; corrected HSA comparison using employer funding and estimated tax benefit rather than treating member contributions as savings | Remaining Core tools still need durable outcomes; Open Enrollment cost-share scenarios remain estimates; authenticated restoration, accuracy, replacement, error, mobile, and accessibility testing remain incomplete |
