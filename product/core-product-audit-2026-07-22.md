...
3. **The Update Engine now has a governed first apply path.** Payday check-ins capture and review an exact take-home change, distinguish a one-time variation from a new recurring baseline, apply only a confirmed recurring value, and preserve an audit trail. Expense, debt, savings, and life changes remain visibly unresolved and route to their authoritative records rather than being guessed. Direct application for those categories and downstream reconciliation into goals, plans, milestones, and Guide follow-up remain.
...
| Update Engine | 82 | B- design | Low-burden payday check-in, durable snapshots, exact recurring-income proposal, side-by-side review, confirmed apply, change history, and honest unresolved routing | Expense, debt, savings, and life changes still require routed completion; no gig prompt, automatic downstream reconciliation, or off-track realignment yet |
