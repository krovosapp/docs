...
**Current overall product-design score:** 92/A-
...
6. **Life Calendar now supports durable one-time and recurring Krovos tasks.** Daily, weekly, biweekly, and monthly tasks are materialized by a daily rolling cron and authenticated lazy future-month top-up, then merged into the existing source-colored calendar. Per-occurrence complete/skip state is independent, archived parents retain completed history, and connected-partner access follows the household/personal/private source split. External imports, native event creation/correction, and notifications remain incomplete.
...
| Life Calendar | 90 | A- | Useful derived events; durable one-time and recurring tasks; daily cron plus authenticated lazy top-up; independent complete/skip history; archive preservation; source-colored layers and task-source-aware connected access | Estimated non-task anchors can look authoritative; no external calendar imports, native event creation/correction, notification workflow, or authenticated mobile/accessibility walkthrough |
...
| Core money tools | 95 | A design | Ten saved/restorable Core outcomes; explicit Paycheck, Open Enrollment, Refinance, Purchase, and rewards-structure choices; interest-bearing-balance safety routing; corrected HSA, refinance, and purchase comparisons; Core-wide Rent vs. Buy persistence | Remaining Core surfaces need final workflow classification; authenticated restoration, calculation-boundary, error, mobile, and accessibility testing remain incomplete |
...
- Feed confirmed changes into a reviewable Life Updates proposal rather than silently changing Life Graph.

### P0: Close the Life Updates loop
...
3. Build proposed-change review and the Life Updates apply path.
