...
4. **Several advertised Core tools are still calculators rather than durable Krovos workflows.** Emergency Fund, Debt Payoff Planner, and Insurance Gap Checker now save and restore completed outcomes in Life Compass. Paycheck Planner, Savings Goals, Open Enrollment Comparison, and other remaining Core tools still need the same durable decision pattern; Savings Goals explicitly states that its data is session-only.
